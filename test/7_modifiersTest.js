...
let closeTime = new Date(2022, 09, 08, 6, 30).getTime(); 
...
describe("Modifiers testing which must reverted with some text.....", () => {
    beforeEach(async () => {
        ...
        it("participating lottery by Manager.... ", async () => {    
            await deployedlottery.applyLottery(appliedTime,{ value:99999999999});
            await expect (deployedlottery.connect(addr2).applyLottery(appliedTime,{ value:99999999999})).to.be.revertedWith("manager cannot participate in lottery system....");
        });
